...
class BayesianOptimization_regression:
    ...
    def __init__(self,TS,CS,AS,x,y,Nmax):
        ...
        # creates necessary variables for handelling the mean structure, these variables are used in the hyperparameter optimization and in the reconstruction (conditional expectations)
        self.ind = len(x)    # index to keep track of what iteration we are at (how many evaluations we currently have)
        self.x = np.zeros([Nmax+ self.ind , self.d+ 1+ sum(CS['beta_quadratic_mask'].copy())]) # allocate memory for x
        self.x[:self.ind,:self.d]
        self.set_regression_structure(CS['beta_linear_mask'].copy(),CS['beta_quadratic_mask'].copy())
        ...
        # initial evaluations
        self.x = np.zeros([Nmax+ self.ind , self.d+ 1+ sum(self.beta_quadratic_mask)]) # allocate memory for x
        self.y = np.zeros([Nmax+self.ind , 1]) # allocate memory for y ( standardized )
        self.y_evals = np.zeros([Nmax+self.ind , 1]) 

        self.x[:self.ind,:] = BayOpt_SideKick.design_matrix(x,self.beta_quadratic_mask) # add quadratic terms and intercept
        self.y_evals[:self.ind] = y                 # save observations
        self.sd = np.std(self.y_evals[:self.ind])   # calculate standard deviation
        self.y = (self.y_evals)/self.sd             # save standardized data for modelling 

        # save the best 10 points for optimization
        I = np.argsort(self.y_evals[0:self.ind],axis= 0).T[0]
        self.y_best_val = self.y_evals[I[:10]]
        self.x_best_val = self.x[I[:10],:self.d]
        self.ymin = self.y_best_val[0] # save the best value so far (to be used by Acquisition functions)
        ...
        self.D[:,:self.ind,:self.ind] = BayOpt_SideKick.difference_matrices_self(self.x[:self.ind,:self.d]) # distance matrix  = sqrt(sum(D),ax=0)
        self.Skk = np.zeros((self.ind+Nmax,self.ind+Nmax)) # covariance between observations
        self.iSr = np.zeros((self.ind,1)) # S_kk^(-1)(Y-XB)
        self.L = np.zeros((self.ind, self.ind)) # cholesky factor

        # GP priors, calculating prior parameters 
...
# Acquisition Optimization ####################################################################################
    ...
    def AF_optimization_sampling(self,nr_sobol_samples_base2 = 4,grad= True): 
        ...
        Skk,invSr = self.Skk[:self.ind,:self.ind], self.iSr[:self.ind]

        
        #x_sample = np.random.uniform(self.domain[:, 0], self.domain[:, 1], size=(n, self.d)) 
        ...
        Af_sample = self.Af(x_sample,Skk,invSr) # evaluate on the samples
        Af_best_sample = self.Af(self.x_best_val[:,:self.d],Skk,invSr)
        ...
        x_best = self.x_best_val[Imin_best,:self.d]
        

        # Acquisition optimization 
        result = minimize(self.Af_g, x_init, bounds=self.domain,jac=grad, method=self.Af_optimizer ,args=(Skk,invSr)) # what if optimization fails 
        result2 = minimize(self.Af_g, x_best, bounds=self.domain,jac=grad, method=self.Af_optimizer ,args=(Skk,invSr)) # what if optimization fails
...
# Save new evaluations ########################################################################################
    def save_eval(self,x, y): #saves the new evaluation point (x,y) and updates the iteration index 
        self.x[self.ind] = BayOpt_SideKick.design_matrix(x,self.beta_quadratic_mask)
        self.y_evals[self.ind] = y
        self.y[self.ind] = (y)/self.sd # save the demeaned observation  

        # update the difference matrix self.D, the covariance matrix self.Skk, the cholesky factor self.L, and S_kk^(-1)Y
        self.update_Skk(self.x[self.ind,:], y/self.sd ,self.ind)

        
        self.y_best_val[-1] = self.y_evals[self.ind] # put the new point as the last point in ymin
        self.x_best_val[-1] = self.x[self.ind,:self.d]
        I = np.argsort(self.y_best_val,axis= 0).T[0] # sort ascending
        self.y_best_val = self.y_best_val[I]
        self.x_best_val = self.x_best_val[I]
        self.ymin = (self.y_best_val[0])/self.sd
        self.ind += 1
...
# Covariance parameters loss & estimation ####################################################
    ...
    def n_loglik_with_approx_gradient(self,par_full,Hutchinson_nr_vec):
        ...
        try: 
            ...
            # precompute Cov_kk^(-1)X
            X_tilde = cho_solve((L,lower),self.x[:ind,self.beta_mask])
            Y_tilde = cho_solve((L,lower),self.y[:ind])
            beta_hat, residuals, rank, s = np.linalg.lstsq(self.x[:self.ind,self.beta_mask].T@X_tilde, self.x[:ind,self.beta_mask].T@Y_tilde, rcond=None)
            XB= X_tilde@beta_hat # X_tilde = Sigma^-1 X
            nlog_p = np.sum(np.log(np.diag(L))) +0.5* (self.y[:ind]-self.x[:ind,self.beta_mask]@beta_hat).T@(Y_tilde-XB)  # negative loglike loss

            YTg = Y_tilde.T @ grad
            grad_YPY =  YTg @(-0.5*Y_tilde + XB) - 0.5*XB.T @ grad @ XB
            g = 0.5*tr.flatten() + (grad_YPY).flatten() # full gradient with respect to the parameters
    ...
    def n_loglik_with_gradient(self,par_full):
        ...
        rho = par_full[0:self.d]    # extract rho
        ind = self.ind
        dist = np.sqrt(np.sum(self.D[:,:ind,:ind] / (rho[:, None, None]**2), axis=0)) # compute distance
        ...
        try: 
            ...
            X_tilde = cho_solve((L,lower),self.x[:ind,self.beta_mask])
            Y_tilde = cho_solve((L,lower),self.y[:ind])
            # profile likelihood estimate of beta (regression parameters)
            # use least squares to estimate beta (this avoid problems with close to singular matrices)
            beta_hat, residuals, rank, s = np.linalg.lstsq(self.x[:self.ind,self.beta_mask].T@X_tilde, self.x[:ind,self.beta_mask].T@Y_tilde, rcond=None)
            XB= X_tilde@beta_hat # X_tilde = Sigma^-1 X
            ...
            YTg = Y_tilde.T @ grad
            grad_YPY =  YTg @(-0.5*Y_tilde + XB) - 0.5*XB.T @ grad @ XB
            g = tr.flatten() + (grad_YPY).flatten() # full gradient with respect to the parameters
...
## Acquisition Functions (Afs) ##################################################################
    ...
    def LCB(self,x,Skk,invSr): # lower confidence bound acquisition, expects x to be 2D array (columns = different dimensions)
        x= BayOpt_SideKick.design_matrix(x,self.beta_quadratic_mask)
        y_rec,y_var = self.reconstruction_Skk(x,Skk,invSr)
        return y_rec-self.Af_param*np.sqrt(y_var)

    # TODO: account for mean structure in gradients
    def LCB_with_gradient(self,x,Skk,invSr): # ( Lower confidence bound)
        # compute reconstruction on x and precompute some other expressions
        x= BayOpt_SideKick.design_matrix(x,self.beta_quadratic_mask)
        y_rec,y_var,g_mu, g_s2 = self.reconstruction_point(x,Skk,invSr)

        g= g_mu -self.Af_param* (g_s2)/(2*np.sqrt(y_var)) 
        return y_rec-self.Af_param*np.sqrt(y_var), g.flatten()
    #--------------------------------------------------------------------------
    def EI(self,x,Skk,invSr): # Expected improvement acquisition, expects x \in R^d, 2D array with (columns = different dimensions)
        ...
        return l
    def EI_contextual(self,x,Skk,invSr): # Expected improvement acquisition, expects x \in R^d, 2D array with (columns = different dimensions)
        # compute reconstruction mean and variance
        x= BayOpt_SideKick.design_matrix(x,self.beta_quadratic_mask)
        y_rec,y_var = self.reconstruction_Skk(x,Skk,invSr) 
    ...
    def EI_with_gradient(self,x,Skk,invSr): # Expected improvement acquisition, works only for one point in R^d, expects a 2D array (row vector)
        # compute reconstruction on x and precompute some other expressions
        x= BayOpt_SideKick.design_matrix(x,self.beta_quadratic_mask)
        y_rec,y_var,g_mu,g_s2 = self.reconstruction_point(x,Skk,invSr)
        y_sig = np.sqrt(y_var)
        ...
        g= g_mu*norm.cdf(z) - norm.pdf(z)*g_s2/(2*y_sig )   
        return l,g
    #--------------------------------------------------------------------------
    def PI(self,x,Skk,invSr): # Probability of improvement acquisition, expects 2D array 
        # compute reconstruction mean and variance
        x= BayOpt_SideKick.design_matrix(x,self.beta_quadratic_mask)
        y_rec,y_var = self.reconstruction_Skk(x,Skk,invSr)
        # scaled improvement
    ...
    def PI_with_gradient(self,x,Skk,invSY):
        # compute reconstruction mean and variance
        x= BayOpt_SideKick.design_matrix(x,self.beta_quadratic_mask)
        y_rec,y_var,g_mu,g_s2 = self.reconstruction_point(x,Skk,invSY)
        sig = np.sqrt(y_var)
        z= (self.ymin- y_rec-self.Af_param)/(sig)    # scaled improvement            
        # gradient for optimization
        g= (g_mu/sig  + z*(g_s2)/(2*y_var))*norm.pdf(z)   
        return - norm.cdf(z),g
    #--------------------------------------------------------------------------
    


    def logPI(self,x,Skk,invSr):
        x= BayOpt_SideKick.design_matrix(x,self.beta_quadratic_mask)
        y_rec,y_var = self.reconstruction_Skk(x,Skk,invSr)
    ...
    def logPI_with_gradient(self,x,Skk,invSY):
        ...
        z= (self.ymin- y_rec-self.Af_param)/(np.sqrt(y_var))                    #
        z2 = - z/np.sqrt(2)
        dz= -(g_mu/np.sqrt(y_var)  + z*(g_s2)/(2*y_var))       
    ...
    def logEI_with_gradient(self,x,Skk,invSY):
        ...
        dz = -(g_mu/y_sig  + z*(g_s2)/(2*y_var))
        nlogei_grad = - dz*self.d_logh(z) -(g_s2) /(2*y_var)
...
# Reconstruction ################################################################################################
    ...
    def reconstruction_Skk(self,xu,Skk,invSr):
        ...
        Duk = BayOpt_SideKick.difference_matrices(xu[:,:self.d],self.x[:self.ind,:self.d]) 
        Duk = np.sqrt(np.sum(Duk / (rho[:, None, None]**2), axis=0))    
        Suk = self.Cf(Duk,*cov_par )

        m_u = (xu[:,self.beta_mask]@self.beta) + Suk@invSr
        v_u = cov_par[0] - np.sum((solve_triangular(self.L,Suk.T,lower= True)**2),axis= 0)
        v_u = np.maximum(v_u,0)
        v_u += sig_ep2
        return m_u,v_u.reshape(-1,1)
    
    def point_reconstruction(self,xu,Skk,invSr): # recurrent function (internal optimization) = > ensure numerical efficiency and stability
        """Performs kriging for a single point (to be used in Acquisition optimization)"""
        rho, cov_par, sig_ep2 = self.extract_param()

        # compute Suk: cross covariance matrix
        Duk = BayOpt_SideKick.distance_anisotropic(xu[:,:self.d],self.x[:self.ind,:self.d],rho) # check what dim xu is
        ...
        Grad_Suk =(grad_Suk_div_Duk)*((xu[:,:self.d]-self.x[:self.ind,:self.d])/(rho**2)).T 

        # gradient of LCB with respect to x
        g_XB =  np.zeros((self.d,1)) # gradient of the linear regression part
        beta = self.beta[:,0]
        xu_vec = xu[0,:self.d]
        if self.n_l:
            g_XB[self.beta_linear_mask,0] = beta[:self.n_l]
        if self.n_q:
            g_XB[self.beta_quadratic_mask,0] += 2*beta[self.n_l+1:]*xu_vec[self.beta_quadratic_mask]
        g_s2 = -2*Grad_Suk@(Skk_inv_SukT)
        g_mu = g_XB + Grad_Suk@(invSr)

        return m_u, v_u, g_mu , g_s2
    

    def point_reconstruction_exp(self,xu,Skk,invSr): # recurrent function (internal optimization) = > ensure numerical efficiency and stability
        """Performs kriging for a single point (to be used in Acquisition optimization)"""
        rho, cov_par, sig_ep2 = self.extract_param()

        # compute Suk: cross covariance matrix
        Duk = BayOpt_SideKick.distance_anisotropic(xu[:,:self.d],self.x[:self.ind,:self.d],rho) 
        ...
        try:
            with np.errstate(divide='raise', invalid='raise'):
                Grad_Suk =(guk/Duk)*((xu[:,:self.d]-self.x[:self.ind,:self.d])/rho).T
     
        except FloatingPointError:
            Grad_Suk= (guk)*((np.ones((1,self.d)))/rho).T
        ...
        if self.n_l:
            g_XB[self.beta_linear_mask,0] = beta[:self.n_l]
        if self.n_q:
            g_XB[self.beta_quadratic_mask,:] += 2*beta[self.n_l+1:]*xu_vec[self.beta_quadratic_mask]
        g_s2 = -2*Grad_Suk@(Skk_inv_SukT)
        g_mu = g_XB + Grad_Suk@(invSr)
    ...
    def compute_Skk_iSY(self): 
        ...
        try: # faster routine
            ...
            X_tilde = cho_solve((L,True),self.x[:ind,self.beta_mask])
            Y_tilde = cho_solve((L,True),self.y[:ind])
            self.beta, residuals, rank, s = np.linalg.lstsq(self.x[:ind,self.beta_mask].T@X_tilde, self.x[:ind,self.beta_mask].T@Y_tilde, rcond=None)
            self.iSr = cho_solve((L,True),self.y[:self.ind]- self.x[:ind, self.beta_mask]@self.beta) # precompute S_kk^{-1} (Y-XB)
        except la.LinAlgError:
            print('cholesky failed: needs handeling')
            self.Lf = False
            self.iSr = np.linalg.lstsq(Skk, self.y[:self.ind]- self.x[:ind, self.beta_mask]@self.beta, rcond=None)

        return None
        

    def update_Skk(self,x_new,y_new,ind):
        """Update cholesky factor, Skk, iSY"""

        rho, cov_par, sig_ep2 = self.extract_param()

        Dnk = BayOpt_SideKick.difference_matrices(x_new[:self.d],self.x[:ind,:self.d])
        ...
        r_new = y_new- x_new[self.beta_mask]@self.beta
        dev = (np.dot(Snk,self.iSr)-r_new)*(1/(sig_ep2 + cov_par[0]- np.dot(Snk,iSkSkn)))
        iSr = np.zeros((ind+1,1))
        iSr[:ind] = self.iSr + iSkSkn*dev 
        iSr[ind] = -dev
        self.iSr = iSr
...
############################################## priors for the GP parameters
    ...
    def set_regression_structure(self,linear_mask, quadratic_mask):
        self.beta_linear_mask = linear_mask # masks to define what regression coefficients to fit
        self.beta_quadratic_mask = quadratic_mask
        self.beta = np.array([np.zeros(1 + sum(self.beta_linear_mask) + sum(self.beta_quadratic_mask))]).T
        self.beta_mask = np.concat((self.beta_linear_mask,np.array([True for i in range(sum(self.beta_quadratic_mask)+1)])) )
        self.n_l = np.sum(self.beta_linear_mask)
        self.n_q = np.sum(self.beta_quadratic_mask)

        x = np.zeros([self.x.shape[0] , self.d+ 1+ sum(self.beta_quadratic_mask)]) # allocate memory for x
        # override x with a new design matrix
        x[:self.ind,:] = BayOpt_SideKick.design_matrix(self.x[:self.ind,:self.d],self.beta_quadratic_mask) # add quadratic terms and intercept
        self.x = x
        return None
