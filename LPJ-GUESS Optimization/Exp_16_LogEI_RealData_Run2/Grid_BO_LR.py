...
#domain = np.array([(0.01, 1), (0.01, 0.07), (0.25, 1), (0.1, 1), (0.001, 0.015), 
#                   (0.8, 1), (0.85, 1), (0, 0.15), (0, 0.1), (0.1, 26)])
...
#--------------------------------------------------
# Initial values
result = optimizer.update_GP_SGD(nr_iter=100,learn_rate=0.01,nr_stc_v_Hutch_tr=150,tolerance=1e-2, adams_patience=5) # updates the GP parameter estimates, does not need to be updated every iteration
...
for k in range(1,Nmax):
    ...
    if (k%n_update == 0) or (k <= 300): # update hyperparameters every 100 iterations
        result = optimizer.update_GP_SGD(nr_iter=50,learn_rate=0.01,nr_stc_v_Hutch_tr=150,tolerance=1e-2, adams_patience=5) # updates the GP parameter estimates, does not need to be updated every iteration
        optimizer.compute_Skk_iSY() # compute new cov matrix and cholesky factor
    ...
    if (k == 1000) or (k == 2000): #full restart at 1000 and compare with current result
        ...
        result2 = optimizer.update_GP_SGD(nr_iter=150,learn_rate=0.01,nr_stc_v_Hutch_tr=150,tolerance=1e-2, adams_patience=5)
        r = 0
...
with open("Grid_BO_10dim_real_data_LR.txt", "w") as file:
...
plt.savefig("Improvement_10dim_real_data.png")
plt.close('all')
...
plt.ylabel('CH4')
plt.legend()
plt.savefig("Time_Series_10dim_real_data.png")
plt.close('all')
...
plt.xlabel('Year')
plt.ylabel('Residual')
plt.savefig("Residuals_10dim_real_data.png")
plt.close('all')
...
with open("BO_execution_time_10dim_real_data.txt", "w") as file:
