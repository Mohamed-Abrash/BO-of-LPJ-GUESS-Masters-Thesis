...
for k in range(1,Nmax):
    ...
    if k == 1500:
        optimizer.set_regression_structure(np.array([True for i in range(optimizer.d)]), np.array([True for i in range(optimizer.d)]))
        result = optimizer.update_GP_SGD(nr_iter=50,learn_rate=0.01,nr_stc_v_Hutch_tr=100,tolerance=1e-2, adams_patience=5) # updates the GP parameter estimates, does not need to be updated every iteration
        optimizer.compute_Skk_iSY() # compute new cov matrix and cholesky factor
