...
from bayesian_optimization_light import BayesianOptimization_light as BO
...
def TF(x):
    # Achley
    #return 20 + np.sum((x)**2 - 10*np.cos(2*np.pi*(x)),axis= x.ndim-1)
    # Spherical
...
CS = {'Cf': 'Matern25', # Covariance function: Matern05, Matern15, Matern25, Gaussian
      ...
      'fixed_param': np.array([*np.zeros(d), 1,1,1]), # fixed parameters: 0 free, 1 fixed parameter
      'optimizer': 'L-BFGS-B',
      'priors': 'logNormal',
...
AS = {'Af': 'logPI', # Acquisition function: EI, PI, LCB
    'Af_param': 2.5, # Exploration parameter
    'optimizer':'L-BFGS-B' # Optimizer choice of acquisition 
    }
#==================================================================

Nmax = 5000
n_update = 200 # reestimate cov parameters every n_update iterations
...
x_,res = optimizer.AF_optimization(9) # returns the next evaluation point
...
for k in range(1,Nmax):
    ...
    # BO iteration
    if (k%n_update == 0) or (k < 200): # update hyperparameters every 100 iterations
        result = optimizer.update_GP_SGD(nr_iter=20,learn_rate=0.01,nr_stc_v_Hutch_tr=100,tolerance=1e-2, adams_patience=5) # updates the GP parameter estimates, does not need to be updated every iteration
        optimizer.compute_Skk_iSY() # compute new cov matrix and cholesky factor
        ...
        status2 =f"Iteration: {k},{result[2]} Adams steps, gradient norm {result[1]}, current minimum {y_best}, current minimum location {optimizer.x_best_val[0]}, current parameters {optimizer.param}"
        #status_update(status1,filename)
        status_update(status2, filename)
        r = 0

    
    if k == 1000: #full restart at 1000 and compare with current result
    ...
    mu,v=optimizer.reconstruction_Skk(x_,optimizer.Skk[:optimizer.ind,:optimizer.ind],optimizer.iSY)
    y_ = TF(x_) # evaluate the new point
    y_std = (y_- optimizer.m)/optimizer.sd
    r += (mu-y_std)**2/v
    R[k] = r
...
with open("Grid_BO_10dim_twin_exp2.txt", "w") as file:
...
np.save('y_evals',optimizer.y_evals)
np.save('x_evals',optimizer.x)
np.save('Chi_Square_resid', R)

Value_recon = Plot_Value(optimizer.x[np.argmin(optimizer.y_evals[:optimizer.ind])])
...
plt.ylabel('CH4')
plt.legend()
plt.savefig("Time_Series_10dim_twin_exp2.png")
plt.close('all')
...
plt.xlabel('Year')
plt.ylabel('Residual')
plt.savefig("Residuals_10dim_twin_exp2.png")
plt.close('all')
...
plt.savefig("Improvement_10dim_twin_exp2.png")
plt.close('all')
'''
...
with open("BO_execution_time_10dim_twin_exp2.txt", "w") as file:
